...
{
	_captureArea = new Object2D(new Custom::Rectangle(center.Get(0), center.Get(1), physicsWidth, physicsHeight), Rigidbody2D(0));
}
...
Position2D DX2DCamera::PhysicsToDisplay(Vector2D absolute)
{
	...
	Custom::Rectangle* captureArea = static_cast<Custom::Rectangle*>(_captureArea->GetVolume());
...
void DX2DCamera::MontageForFilm(DX2DMapObject* map)
{
	...
	map->GetDXObject()->SetCenter(center);
