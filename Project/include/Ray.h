...
namespace SSB
{
	...
	// Assume every position is based on World space
	class Ray
	{
		static const float EndlessLength;

	private:
		Vector3 _origin;
		Vector3 _direction;
		float _length = EndlessLength;

	public:
		Ray(Vector3 origin, Vector3 dir);
		Ray(Vector3 origin, Vector3 dir, float length);

	public:
		bool IsIntersect(Vector3 v1, Vector3 v2, Vector3 v3);
		Vector3 GetIntersectionPoint(Vector3 v1, Vector3 v2, Vector3 v3);
	};
}
